...
int main() {
  ...
  //【4】基于FLANN的描述符对象匹配
  flann::Index flannIndex(descriptors,
  ...
  while (1) {
    ...
    //【9】计算描述符
    featureDetector->compute(captureImage_gray,
                             captureKeyPoints,
                             captureDescription);
    ...
    for (int i = 0; i < matchDistance.rows; i++) {
      if (matchDistance.at<float>(i, 0) < 0.6 * matchDistance.at<float>(i, 1)) {
        DMatch dmatches
            (i, matchIndex.at<int>(i, 0), matchDistance.at<float>(i, 0));
    ...
    //【12】绘制并显示匹配窗口
    Mat resultImage;
    drawMatches(captureImage, captureKeyPoints, srcImage, keyPoints,
                goodMatches, resultImage);
